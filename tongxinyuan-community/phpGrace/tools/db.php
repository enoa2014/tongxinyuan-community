...
class db
{
	...
	public function __construct($conf)
	{
		$this->conf = $conf;
		try {
			$this->pdo = new \PDO("mysql:host={$conf['host']};port={$conf['port']};dbname={$conf['dbname']};charset={$conf['charset']}", $conf['user'], $conf['pwd']);
			$this->pdo->query("set names {$conf['charset']}");
		} catch (Exception $e) {
			throw new \pgException('数据库连接失败', '请检查数据库相关配置');
		}
	}
	...
	public function add($data = null)
	{
		...
		if (!is_array($data)) {
			throw new \pgException('插入数据错误', '插入数据应为一个一维数组');
		}
	...
	{
		if (empty($this->where)) {
			throw new \pgException('请使用模型对象的where()函数设置删除条件');
		}
		$where = $this->getWhere();
	...
	public function update($data = null)
	{
		...
		if (empty($data) || !is_array($data)) {
			throw new \pgException('update($data) 函数的参数应该为一个一维数组');
		}
		if (empty($this->where)) {
			throw new pgException('请使用模型对象的 where() 方法设置更新条件');
		}
		$where = $this->getWhere();
